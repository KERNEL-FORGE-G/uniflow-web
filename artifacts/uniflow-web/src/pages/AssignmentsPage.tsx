    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-2">
        <select
          ...
          className="rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm text-[#374151] outline-none focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/10"
        >
        ...
        >
          ...
          {[...new Set(assignments.map(a => a.code))].map(code => (
        ...
        <select
          ...
          className="rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm text-[#374151] outline-none focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/10"
        >
          <option value="asc">Date ↑ (plus proches)</option>
          <option value="desc">Date ↓ (plus lointaines)</option>
        </select>
        {(filter !== 'Tous' || filterUE) && (
          <button
            onClick={() => { setFilter('Tous'); setFilterUE(''); }}
            className="flex items-center gap-1.5 rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm font-semibold text-[#374151] hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all"
          >
            ✕ Réinitialiser
          </button>
        )}
