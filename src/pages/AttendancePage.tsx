    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white border border-[#e5e7eb] p-5 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => {
              const rows: string[] = ['Cours,Code,Présents,Absences,Retards,Taux']
              myAttendanceByCourse.forEach(c => rows.push(`"${c.course}",${c.code},${c.present},${c.absent},${c.late},${c.rate}%`))
              const csv = rows.join('\n')
              const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
              const url = URL.createObjectURL(blob)
              const a = document.createElement('a')
              a.href = url; a.download = 'mes-presences.csv'; a.click()
